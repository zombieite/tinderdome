...
@if ($upcoming_events_and_signup_status)
    <li><div>
        ...
        <form action="/" method="POST">
            ...
            @foreach ($upcoming_events_and_signup_status as $upcoming_event)
                <input class="upcoming_event_checkbox" type="checkbox" name="attending_event_id_{{ $upcoming_event->event_id }}" @if ($upcoming_event->attending_event_id) @if ($upcoming_event->user_id_of_match) disabled @endif checked @endif > @if ($upcoming_event->user_id_of_match) <a class="bright" href="/profile/match?event={{ $upcoming_event->event_short_name }}&date={{ $upcoming_event->event_date }}"> @endif {{ $upcoming_event->event_long_name }} @if ($upcoming_event->user_id_of_match) </a> @else @if ($upcoming_event->signups_still_needed) &middot; {{ $upcoming_event->attending_count }} signed up, @if ($upcoming_event->signups_still_needed === 1) only one more signup still needed! Tell your friends! @else {{ $upcoming_event->signups_still_needed }} more signups are needed. Tell your friends! @endif @endif @endif <br>
            @endforeach
