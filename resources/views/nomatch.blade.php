...
@if ($matches_done)
	@if ($deleted_match_or_match_said_no)
		<h2>You were matched for {{ $event }} {{ $year }}, but your match has deleted their profile.</h2>
	@else
		<h2>We did not find you a match for {{ $event }} {{ $year }}.</h2>
		<p>
	...
	@endif
@else
	Matches have not been run yet for {{ $event }} {{ $year }}. Check back a few days before the event.
@endif
@endsection
